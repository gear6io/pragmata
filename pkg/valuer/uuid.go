...
func (enum *UUID) Scan(val interface{}) error {
	if enum == nil {
		return errors.New("uuid: (nil \"%s\")" + reflect.TypeOf(enum).String())
	}

	if val == nil {
		return errors.New("uuid: (nil \"%s\")" + reflect.TypeOf(val).String())
	}
	...
	case string:
		...
		if err != nil {
			return errors.New("uuid: (invalid-uuid \"%s\")" + err.Error())
		}
		enumVal = _enumVal
	case []byte:
		...
		enumVal = _enumVal
	default:
		return errors.New("uuid: (non-uuid \"%s\")" + reflect.TypeOf(val).String())
	}
